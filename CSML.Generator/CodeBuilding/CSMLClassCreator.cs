...
internal static class CSMLClassCreator
{
    ...
    private static ITagDeclarationSyntax[] GetTypesToCreateMembersFor(CSMLSyntaxTree syntaxTree)
    {
        return GetTypesEnumerable(syntaxTree).ToArray();

        static IEnumerable<ITagDeclarationSyntax> GetTypesEnumerable(CSMLSyntaxTree syntaxTree)
        {
            var descendingNodes = syntaxTree.GetRoot().DescendingNodes().ToArray();

            foreach (var node in descendingNodes) {
                if (node is ITagDeclarationSyntax x) {
    ...
    private static void AppendMembersOfNamedTags(StringBuilder sb, ITagDeclarationSyntax[] typeToCreateMembersFor)
    {
    ...
    private static void AppendSetupCodeFromNode(StringBuilder sb, TagSelfClosingSyntax syntaxNode, string lastCreatedNode, ref int varCount)
    {
        ...
        if (syntaxNode.DirectChildren.Any()) {
            throw new NotImplementedException("Self closing tag had child nodes which is not implemented yet.");
        }
